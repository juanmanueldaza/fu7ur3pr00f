...
from fu7ur3pr00f.agents.middleware import ToolCallRepairMiddleware
...
class TestToolCallRepairMiddleware:
    ...
    def test_all_tool_calls_matched(self):
        state = _make_state(
            [
                ...
                ToolMessage(content="result", tool_call_id="call_2"),
            ]
        )
        result = self.middleware.before_model(state, self.runtime)
        assert result is None

    def test_mixed_match_and_orphan(self):
        state = _make_state(
            [
                HumanMessage(content="analyze"),
                AIMessage(
                ...
                ToolMessage(content="result", tool_call_id="call_1"),
                # call_2 has no response — should be repaired
            ]
        )
        result = self.middleware.before_model(state, self.runtime)
    ...
    def test_multiple_orphaned_tool_calls(self):
        state = _make_state(
            [
                HumanMessage(content="analyze"),
                AIMessage(
        ...
        assert "call_2" in repair_ids
        assert "call_3" in repair_ids

    def test_no_tool_calls_in_last_ai_message(self):
        state = _make_state(
            [
                HumanMessage(content="analyze"),
                AIMessage(
    ...
    def test_last_ai_with_tool_calls_has_orphans(self):
        """The last AIMessage with tool_calls has orphans, even if not the very last
        message."""
        state = _make_state(
            [
                HumanMessage(content="first"),
